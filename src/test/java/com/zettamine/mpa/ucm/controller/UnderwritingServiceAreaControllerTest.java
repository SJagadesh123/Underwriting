...
public class UnderwritingServiceAreaControllerTest {
	...
	void testSaveUnderwriter() throws Exception {
		...
		// Then
		mockMvc.perform(post("/api/v1/underwriting/service-area/create").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(serviceAreaDto))).andExpect(status().isCreated())
...
//    }
	...
	void testFetch() throws Exception {
		...
		// Then
		mockMvc.perform(get("/api/v1/underwriting/service-area/fetch/{id}", id).contentType(MediaType.APPLICATION_JSON))
	...
	void testFetchAll() throws Exception {
		...
		// Then
		mockMvc.perform(get("/api/v1/underwriting/service-area/fetchAll").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());

	}

	@Test
	void updateUnderwriter_ValidInput_Returns200() throws Exception {
		// Arrange
		Long id = 123L;
		...
		// Act
		mockMvc.perform(MockMvcRequestBuilders.put("/api/v1/underwriting/service-area/update/{id}", id).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(serviceAreaDto)))
				.andExpect(MockMvcResultMatchers.status().isCreated())
				.andExpect(MockMvcResultMatchers.jsonPath("$.statusCode").value(AppConstants.STATUS_200))
				.andExpect(MockMvcResultMatchers.jsonPath("$.statusMsg").value(AppConstants.MESSAGE_200));

		// Assert
		verify(serviceAreaService).update(id, serviceAreaDto);
	}
