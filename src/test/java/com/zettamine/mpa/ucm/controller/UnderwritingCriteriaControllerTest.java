...
public class UnderwritingCriteriaControllerTest {
    ...
    void testSaveCriteria() throws Exception {
        ...
        // Then
        mockMvc.perform(post("/api/v1/underwriting/criteria/create")
    ...
    void testFetchCriteria() throws Exception {
        ...
        // Then
        mockMvc.perform(get("/api/v1/underwriting/criteria/fetch/{id}", criteriaId)
                .contentType(MediaType.APPLICATION_JSON))
    ...
    void testFetchAllCriteria() throws Exception {
        ...
        // Then
        mockMvc.perform(get("/api/v1/underwriting/criteria/fetchAll")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void testAddCriteriaToProd() throws Exception {
        ...
        // When
        doNothing().when(underwritingCriteriaService).addCriteriaToLoanProd(loanProductCriteriaDto.getCriteriaNames(), loanProductCriteriaDto.getProductName());

        // Then
        mockMvc.perform(post("/api/v1/underwriting/criteria/add-criteria-to-loanProd")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void testFetchByCriteriaName() throws Exception {
        ...
        // Then
        mockMvc.perform(get("/api/v1/underwriting/criteria/get-loan-by-criteria-name")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(criteriaNames)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    ...
    void testFetchAllCriteriaNames() throws Exception {
        // Given
        List<String> criteriaNames = Arrays.asList("Credit Score Requirement", "Income Verification", "Debt-to-Income Ratio");
        
        // When
        when(underwritingCriteriaService.getAllCriteriaNames()).thenReturn(criteriaNames);
        
        // Then
        mockMvc.perform(get("/api/v1/underwriting/criteria/fetchAll-criteria-names")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
